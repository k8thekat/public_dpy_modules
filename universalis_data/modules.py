...
from extensions.universalis_data._types import (
...
if TYPE_CHECKING:
    ...
    from extensions.universalis_data._types import GarlandToolsAPI_ItemCraftIngredientsTyped
    from kuma_kuma import Kuma_Kuma

    from ..ffxiv import FFXIV, FFWatchListDBTyped, UniversalisDBTyped
    from ._types import (
        GarlandToolsAPI_FishingLocationsTyped,
...
class FFXIVUser:
    ...
    def __init__(self, data: UniversalisDBTyped, db_pool: asqlite.Pool) -> None:
    ...
    async def get_watch_list_items(self) -> list[FFXIVItemWatchList]:
        async with self.pool.acquire() as conn:
            res: list[FFWatchListDBTyped] = await conn.fetchall("""SELECT * FROM watchlist WHERE discord_id = ? and guild_id = ? RETURNING *""")  # type: ignore
            for item in res:
    ...
    async def update_watch_list_item(self, item: FFXIVItemWatchList) -> FFXIVItemWatchList:
        async with self.pool.acquire() as conn:
            res: FFWatchListDBTyped = await conn.fetchone(  # type: ignore
...
class FFXIVResource:
    ...
    resource_path: Path = Path(__file__).parent.joinpath("resources")
    aethernet_icon: discord.File = discord.File(fp=resource_path.joinpath("aethernet-icon.png"), filename="aethernet-icon.png")
    item_banner: discord.File = discord.File(fp=resource_path.joinpath("ffxiv-trail-banner.png"), filename="ffxiv-banner.png")
    universalis_icon: discord.File = discord.File(fp=resource_path.joinpath("universalis-icon.png"), filename="uni-icon.png")
    garlandtools_icon: discord.File = discord.File(fp=resource_path.joinpath("garlandtools-icon.png"), filename="gt-icon.png")
    ...
    async def get_items(self) -> None:
        ...
        if data.status != 200:
            self.logger.error("We encountered an error retrieving our items.json data. | URL: %s", self.item_url)
            raise ConnectionError("We encountered an error retrieving our items.json data.")

        res: ItemIDTyped = await data.json(content_type="text/plain")
