...
from cogs.love_cog import LoverEntry
...
class ShortTime:
    ...
    # @classmethod
    # async def convert(cls, ctx: KumaContext, argument: str) -> Self:
...
class UserFriendlyTime(commands.Converter):
    ...
    async def convert(self, ctx: Context, argument: str) -> FriendlyTimeResult:
        ...
        # reminder = ctx.bot.reminder
        lover = await LoverEntry.get_or_none(discord_id=ctx.author.id)
        tzinfo = datetime.timezone.utc
        if lover is not None:
            res = await lover.get_timezone()
            if res is not None:
                tzinfo = res["timezone"]
